...
function OrderStatusPill({ status }) {
  return (
    <span
      className={`inline-flex items-center rounded-md px-2 py-0.5 text-[10px] font-semibold ${getOrderStatusClass(status)}`}
    >
  ...
  return (
    <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
      <div className="min-w-0">
        <h2 className="text-sm font-bold tracking-tight text-dash-text sm:text-base">{title}</h2>
        {subtitle ? <p className="mt-0.5 text-xs text-dash-muted">{subtitle}</p> : null}
      </div>
      {action}
    </div>
  );
                    ...
                    <table className="min-w-full border-collapse text-left text-sm">
                      <thead>
                        <tr className="border-b border-dash-border bg-slate-50/90">
                          <th className="px-5 py-2.5 text-[10px] font-semibold tracking-[0.08em] text-dash-muted uppercase">
                            Order
                          </th>
                          <th className="px-4 py-2.5 text-[10px] font-semibold tracking-[0.08em] text-dash-muted uppercase">
                            Customer
                          </th>
                          <th className="px-4 py-2.5 text-[10px] font-semibold tracking-[0.08em] text-dash-muted uppercase">
                            Amount
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {recentOrders.map((order) => (
                          ...
                          >
                            ...
                            <td className="px-5 py-2.5 text-[11px] text-dash-muted">
                ...
                <div className="space-y-2.5">
                  {activities.map((item) => {
                    ...
                    return (
                      <div
                        key={item.id}
                        className="flex gap-2.5 rounded-md border border-dash-border/80 bg-slate-50/60 px-3 py-2.5"
                      >
                ...
                {quickActions.map((action) => {
                  ...
                  return (
                    <Link
                      key={action.label}
                      href={action.href}
                      className="group flex items-center gap-3 rounded-md border border-dash-border bg-white px-3 py-2.5 transition-colors hover:border-indigo-200 hover:bg-indigo-50/40"
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="text-[11px] text-dash-muted">{action.desc}</span>
                      </span>
        ...
        {[
          ...
          <MotionFadeIn key={metric.label} delay={320 + index * 40}>
            <div className={`dash-card border px-4 py-3.5 ${metric.cardClass}`}>
              <p className="text-[10px] font-semibold tracking-[0.08em] text-dash-muted uppercase">
                {metric.label}
              </p>
              <p className={`mt-1 text-lg font-bold tabular-nums ${metric.valueClass}`}>
                {metric.value}
              </p>
              <p className="mt-0.5 text-[11px] text-dash-muted">{metric.hint}</p>
            </div>
          </MotionFadeIn>
        ))}
      </section>
